...
function formatBytes(bytes, decimals = 2, rawDict) {
    ...
    const sizes = rawDict?.byteUnits || ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    return (
        <div className="apps-modal">
            ...
            <div className="apps-modal-content">
                {isLoading && <div className="apps-modal-loading"><SpinnerIcon /><p>{t('appsModal.loading', 'Loading apps...')}</p></div>}
                {error && <p className="apps-modal-error">{error}</p>}
                {!isLoading && !error && appData && (
                    <>
                        {selectedApp ? (
                            <div className="app-detail-view">
                                <button onClick={handleBackToGrid} className="app-detail-back-button">&larr; {t('appsModal.backButton', 'Back to list')}</button>
                                <img src={`${IMAGE_BASE_URL}${selectedApp.icon}`} alt={selectedApp.full_name} className="app-detail-icon" onError={(e) => { e.target.style.display = 'none'; }} />
                                ...
                                <div className="app-action-buttons">
                                    {isAppInstalled(selectedApp.name) ? (
                                        <>
                                            <button onClick={() => handleUpdate(selectedApp.name)} className="app-action-button update">{t('appsModal.updateButton', 'Update')} {selectedApp.name}</button>
                                            <button onClick={() => handleRemove(selectedApp.name)} className="app-action-button remove">{t('appsModal.removeButton', 'Remove')} {selectedApp.name}</button>
                                        </>
                                    ) : (
                                        <button onClick={() => handleInstall(selectedApp.name)} className="app-action-button install">{t('appsModal.installButton', 'Install')} {selectedApp.name}</button>
                                    )}
                                </div>
                            </div>
                        ) : (
                            <>
                                <input type="text" className="apps-search-bar allow-native-input" placeholder={t('appsModal.searchPlaceholder', "Search apps...")} value={searchTerm} onChange={handleSearchChange} />
                                <div className="apps-grid">
                                    {filteredApps.length > 0 ? filteredApps.map(app => (
                                        <div key={app.name} className="app-card" onClick={() => handleAppClick(app)}>
                                            <img src={`${IMAGE_BASE_URL}${app.icon}`} alt={app.full_name} className="app-card-icon" loading="lazy" onError={(e) => { e.target.style.visibility = 'hidden'; }} />
...
function Sidebar({ isOpen }) {
  ...
  useEffect(() => {
    ...
    if (navigator.userAgentData && navigator.userAgentData.mobile !== undefined) {
        console.log('Dashboard: Mobile detected via userAgentData.mobile:', navigator.userAgentData.mobile);
  ...
  const populateAudioDevices = useCallback(async () => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError') userMessageKey = 'sections.audio.deviceErrorPermission';
      else if (err.name === 'NotFoundError') userMessageKey = 'sections.audio.deviceErrorNotFound';
  ...
  const handleEncoderChange = (event) => {
    ...
    setEncoder(selectedEncoder); localStorage.setItem('encoder', selectedEncoder);
  ...
  const handleAudioOutputChange = (event) => {
      const deviceId = event.target.value; setSelectedOutputDeviceId(deviceId);
      window.postMessage({ type: 'audioDeviceSelected', context: 'output', deviceId: deviceId }, window.location.origin);
  };
  const handlePresetChange = (event) => {
      const selectedValue = event.target.value; setPresetValue(selectedValue);
      if (!selectedValue) return;
      const parts = selectedValue.split('x');
      if (parts.length === 2) {
          const width = parseInt(parts[0], 10), height = parseInt(parts[1], 10);
          if (!isNaN(width) && width > 0 && !isNaN(height) && height > 0) {
              const evenWidth = roundDownToEven(width), evenHeight = roundDownToEven(height);
              setManualWidth(evenWidth.toString()); setManualHeight(evenHeight.toString());
              window.postMessage({ type: 'setManualResolution', width: evenWidth, height: evenHeight }, window.location.origin);
          } else console.error("Dashboard: Error parsing selected resolution preset:", selectedValue);
      }
  };
  const handleManualWidthChange = (event) => { setManualWidth(event.target.value); setPresetValue(""); };
  const handleManualHeightChange = (event) => { setManualHeight(event.target.value); setPresetValue(""); };
  const handleScaleLocallyToggle = () => {
      const newState = !scaleLocally; setScaleLocally(newState); localStorage.setItem('scaleLocallyManual', newState.toString());
      window.postMessage({ type: 'setScaleLocally', value: newState }, window.location.origin);
  };
  const handleSetManualResolution = () => {
      const width = parseInt(manualWidth.trim(), 10), height = parseInt(manualHeight.trim(), 10);
      if (isNaN(width) || width <= 0 || isNaN(height) || height <= 0) {
          alert(t('alerts.invalidResolution')); return;
      }
      const evenWidth = roundDownToEven(width), evenHeight = roundDownToEven(height);
      setManualWidth(evenWidth.toString()); setManualHeight(evenHeight.toString()); setPresetValue("");
      window.postMessage({ type: 'setManualResolution', width: evenWidth, height: evenHeight }, window.location.origin);
  };
  ...
  const handleVideoToggle = () => window.postMessage({ type: 'pipelineControl', pipeline: 'video', enabled: !isVideoActive }, window.location.origin);
  const handleAudioToggle = () => window.postMessage({ type: 'pipelineControl', pipeline: 'audio', enabled: !isAudioActive }, window.location.origin);
  const handleMicrophoneToggle = () => window.postMessage({ type: 'pipelineControl', pipeline: 'microphone', enabled: !isMicrophoneActive }, window.location.origin);
  const handleGamepadToggle = () => window.postMessage({ type: 'gamepadControl', enabled: !isGamepadEnabled }, window.location.origin);
  const handleFullscreenRequest = () => window.postMessage({ type: 'requestFullscreen' }, window.location.origin);
  const handleClipboardChange = (event) => setDashboardClipboardContent(event.target.value);
  const handleClipboardBlur = (event) => window.postMessage({ type: 'clipboardUpdateFromUI', text: event.target.value }, window.location.origin);
  ...
  const getTooltipContent = useCallback((itemKey) => {
      ...
      switch (itemKey) {
          case 'cpu': return t('sections.stats.tooltipCpu', { value: cpuPercent.toFixed(1) });
          case 'gpu': return t('sections.stats.tooltipGpu', { value: gpuPercent.toFixed(1) });
          case 'sysmem':
              const fu = sysMemUsed !== null ? formatBytes(sysMemUsed, 2, raw) : memNA;
              const ft = sysMemTotal !== null ? formatBytes(sysMemTotal, 2, raw) : memNA;
              return (fu !== memNA && ft !== memNA) ? t('sections.stats.tooltipSysMem', { used: fu, total: ft }) : `${t('sections.stats.sysMemLabel')}: ${memNA}`;
          case 'gpumem':
              const gu = gpuMemUsed !== null ? formatBytes(gpuMemUsed, 2, raw) : memNA;
              const gt = gpuMemTotal !== null ? formatBytes(gpuMemTotal, 2, raw) : memNA;
              return (gu !== memNA && gt !== memNA) ? t('sections.stats.tooltipGpuMem', { used: gu, total: gt }) : `${t('sections.stats.gpuMemLabel')}: ${memNA}`;
          case 'fps': return t('sections.stats.tooltipFps', { value: clientFps });
          case 'audio': return t('sections.stats.tooltipAudio', { value: audioBuffer });
          default: return '';
      }
  }, [t, raw, cpuPercent, gpuPercent, sysMemUsed, sysMemTotal, gpuMemUsed, gpuMemTotal, clientFps, audioBuffer]);
  ...
  const scheduleNotificationRemoval = useCallback((id, delay) => {
    ...
    const fadeTimer = setTimeout(() => setNotifications(prev => prev.map(n => n.id === id ? { ...n, fadingOut: true } : n)), delay - NOTIFICATION_FADE_DURATION);
    const removeTimer = setTimeout(() => removeNotification(id), delay);
    notificationTimeouts.current[id] = { fadeTimer, removeTimer };
  }, [removeNotification]);

  const handleUploadClick = () => window.dispatchEvent(new CustomEvent('requestFileUpload'));

  useEffect(() => {
    const savedEncoder = localStorage.getItem('encoder');
    if (savedEncoder && encoderOptions.includes(savedEncoder)) setEncoder(savedEncoder); else { setEncoder(DEFAULT_ENCODER); localStorage.setItem('encoder', DEFAULT_ENCODER); }
  ...
  useEffect(() => {
    const readStats = () => {
        const cs = window.system_stats, su = cs?.mem_used ?? null, st = cs?.mem_total ?? null;
        setCpuPercent(cs?.cpu_percent ?? 0); setSysMemUsed(su); setSysMemTotal(st);
        setSysMemPercent((su !== null && st !== null && st > 0) ? (su / st) * 100 : 0);
        const cgs = window.gpu_stats, gp = cgs?.gpu_percent ?? cgs?.utilization_gpu ?? 0;
        setGpuPercent(gp);
        const gu = cgs?.mem_used ?? cgs?.memory_used ?? cgs?.used_gpu_memory_bytes ?? null;
        const gt = cgs?.mem_total ?? cgs?.memory_total ?? cgs?.total_gpu_memory_bytes ?? null;
        setGpuMemUsed(gu); setGpuMemTotal(gt);
        setGpuMemPercent((gu !== null && gt !== null && gt > 0) ? (gu / gt) * 100 : 0);
        setClientFps(window.fps ?? 0); setAudioBuffer(window.currentAudioBufferSize ?? 0);
    };
        ...
        if (typeof message === 'object' && message !== null) {
            if (message.type === 'pipelineStatusUpdate') {
              ...
              if (message.microphone !== undefined) setIsMicrophoneActive(message.microphone);
               ...
               if (message.enabled !== undefined) setIsGamepadEnabled(message.enabled);
              ...
              if (message.gamepad !== undefined) setIsGamepadEnabled(message.gamepad);
                ...
                if (typeof message.text === 'string') setDashboardClipboardContent(message.text);
                ...
                const gpIndex = message.gamepadIndex; if (gpIndex === undefined || gpIndex === null) return;
                ...
                const { status, fileName, progress, fileSize, message: errMsg } = message.payload; const id = fileName;
                setNotifications(prev => {
                    const exIdx = prev.findIndex(n => n.id === id);
                    if (status === 'start') {
                        if (prev.length < MAX_NOTIFICATIONS && exIdx === -1) return [...prev, { id, fileName, status: 'progress', progress: 0, fileSize, message: null, timestamp: Date.now(), fadingOut: false }];
                        else return prev;
                    } else if (exIdx !== -1) {
                        const un = [...prev], cn = un[exIdx];
                        ...
                        if (status === 'progress') un[exIdx] = { ...cn, status: 'progress', progress, timestamp: Date.now(), fadingOut: false };
                        else if (status === 'end') { un[exIdx] = { ...cn, status: 'end', progress: 100, message: null, timestamp: Date.now(), fadingOut: false }; scheduleNotificationRemoval(id, NOTIFICATION_TIMEOUT_SUCCESS); }
                        else if (status === 'error') { const te = errMsg ? `${t('notifications.errorPrefix')} ${errMsg}` : t('notifications.unknownError'); un[exIdx] = { ...cn, status: 'error', progress: 100, message: te, timestamp: Date.now(), fadingOut: false }; scheduleNotificationRemoval(id, NOTIFICATION_TIMEOUT_ERROR); }
                        return un;
                    } else return prev;
                });
  ...
  const gaugeSize = 80, gaugeStrokeWidth = 8, gaugeRadius = (gaugeSize / 2) - (gaugeStrokeWidth / 2);
  const gaugeCircumference = 2 * Math.PI * gaugeRadius, gaugeCenter = gaugeSize / 2;
  ...
  return (
    <>
      <div className={sidebarClasses}>
        <div className="sidebar-header">
           ...
           <div className="header-controls">
             <div className={`theme-toggle ${theme}`} onClick={toggleTheme} title={t('toggleThemeTitle')}>
               <svg className="icon moon-icon" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>
               <svg className="icon sun-icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>
             </div>
        ...
        <div className="sidebar-section">
            <div className="sidebar-section-header" onClick={() => toggleSection('settings')} role="button" aria-expanded={sectionsOpen.settings} aria-controls="settings-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('settings')}>
              <h3>{t('sections.video.title')}</h3> <span className="section-toggle-icon">{sectionsOpen.settings ? <CaretUpIcon /> : <CaretDownIcon />}</span>
            </div>
        ...
        <div className="sidebar-section">
             <div className="sidebar-section-header" onClick={() => toggleSection('audioSettings')} role="button" aria-expanded={sectionsOpen.audioSettings} aria-controls="audio-settings-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('audioSettings')}>
               <h3>{t('sections.audio.title')}</h3> <span className="section-toggle-icon">{isLoadingAudioDevices ? <SpinnerIcon /> : (sectionsOpen.audioSettings ? <CaretUpIcon /> : <CaretDownIcon />)}</span>
             </div>
        ...
        <div className="sidebar-section">
             <div className="sidebar-section-header" onClick={() => toggleSection('screenSettings')} role="button" aria-expanded={sectionsOpen.screenSettings} aria-controls="screen-settings-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('screenSettings')}>
               <h3>{t('sections.screen.title')}</h3> <span className="section-toggle-icon">{sectionsOpen.screenSettings ? <CaretUpIcon /> : <CaretDownIcon />}</span>
             </div>
             {sectionsOpen.screenSettings && (
               <div className="sidebar-section-content" id="screen-settings-content">
                 <div className="dev-setting-item"> <label htmlFor="resolutionPresetSelect">{t('sections.screen.presetLabel')}</label> <select id="resolutionPresetSelect" value={presetValue} onChange={handlePresetChange}> {translatedCommonResolutions.map((res, i) => (<option key={i} value={res.value} disabled={i === 0}>{res.text}</option>))} </select> </div>
                 <div className="resolution-manual-inputs">
                    <div className="dev-setting-item manual-input-item"> <label htmlFor="manualWidthInput">{t('sections.screen.widthLabel')}</label> <input className="allow-native-input" type="number" id="manualWidthInput" min="1" step="2" placeholder={t('sections.screen.widthPlaceholder')} value={manualWidth} onChange={handleManualWidthChange} /> </div>
                    <div className="dev-setting-item manual-input-item"> <label htmlFor="manualHeightInput">{t('sections.screen.heightLabel')}</label> <input className="allow-native-input" type="number" id="manualHeightInput" min="1" step="2" placeholder={t('sections.screen.heightPlaceholder')} value={manualHeight} onChange={handleManualHeightChange} /> </div>
                 </div>
                 <div className="resolution-action-buttons"> <button className="resolution-button" onClick={handleSetManualResolution}>{t('sections.screen.setManualButton')}</button> <button className="resolution-button reset-button" onClick={handleResetResolution}>{t('sections.screen.resetButton')}</button> </div>
                 <button className={`resolution-button toggle-button ${scaleLocally ? 'active' : ''}`} onClick={handleScaleLocallyToggle} style={{ marginTop: '10px' }} title={t(scaleLocally ? 'sections.screen.scaleLocallyTitleDisable' : 'sections.screen.scaleLocallyTitleEnable')}> {t('sections.screen.scaleLocallyLabel')} {t(scaleLocally ? 'sections.screen.scaleLocallyOn' : 'sections.screen.scaleLocallyOff')} </button>
               </div>
             )}
        </div>
        ...
        <div className="sidebar-section">
             <div className="sidebar-section-header" onClick={() => toggleSection('stats')} role="button" aria-expanded={sectionsOpen.stats} aria-controls="stats-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('stats')}>
               <h3>{t('sections.stats.title')}</h3> <span className="section-toggle-icon">{sectionsOpen.stats ? <CaretUpIcon /> : <CaretDownIcon />}</span>
             </div>
             {sectionsOpen.stats && (
               <div className="sidebar-section-content" id="stats-content">
                 <div className="stats-gauges">
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'cpu')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: cpuOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {Math.round(Math.max(0, Math.min(100, cpuPercent || 0)))}% </text> </svg> <div className="gauge-label">{t('sections.stats.cpuLabel')}</div> </div>
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'gpu')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: gpuOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {Math.round(Math.max(0, Math.min(100, gpuPercent || 0)))}% </text> </svg> <div className="gauge-label">{t('sections.stats.gpuLabel')}</div> </div>
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'sysmem')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: sysMemOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {Math.round(Math.max(0, Math.min(100, sysMemPercent || 0)))}% </text> </svg> <div className="gauge-label">{t('sections.stats.sysMemLabel')}</div> </div>
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'gpumem')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: gpuMemOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {Math.round(Math.max(0, Math.min(100, gpuMemPercent || 0)))}% </text> </svg> <div className="gauge-label">{t('sections.stats.gpuMemLabel')}</div> </div>
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'fps')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: fpsOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {clientFps} </text> </svg> <div className="gauge-label">{t('sections.stats.fpsLabel')}</div> </div>
                    <div className="gauge-container" onMouseEnter={(e) => handleMouseEnter(e, 'audio')} onMouseLeave={handleMouseLeave}> <svg width={gaugeSize} height={gaugeSize} viewBox={`0 0 ${gaugeSize} ${gaugeSize}`}> <circle stroke="var(--item-border)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} /> <circle stroke="var(--sidebar-header-color)" fill="transparent" strokeWidth={gaugeStrokeWidth} r={gaugeRadius} cx={gaugeCenter} cy={gaugeCenter} transform={`rotate(-90 ${gaugeCenter} ${gaugeCenter})`} style={{ strokeDasharray: gaugeCircumference, strokeDashoffset: audioBufferOffset, transition: 'stroke-dashoffset 0.3s ease-in-out', strokeLinecap: 'round' }} /> <text x={gaugeCenter} y={gaugeCenter} textAnchor="middle" dominantBaseline="central" fontSize={`${gaugeSize / 5}px`} fill="var(--sidebar-text)" fontWeight="bold"> {audioBuffer} </text> </svg> <div className="gauge-label">{t('sections.stats.audioLabel')}</div> </div>
                 </div>
               </div>
             )}
        </div>
        ...
        <div className="sidebar-section">
             <div className="sidebar-section-header" onClick={() => toggleSection('clipboard')} role="button" aria-expanded={sectionsOpen.clipboard} aria-controls="clipboard-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('clipboard')}>
               <h3>{t('sections.clipboard.title')}</h3> <span className="section-toggle-icon">{sectionsOpen.clipboard ? <CaretUpIcon /> : <CaretDownIcon />}</span>
             </div>
             {sectionsOpen.clipboard && ( <div className="sidebar-section-content" id="clipboard-content"> <div className="dashboard-clipboard-item"> <label htmlFor="dashboardClipboardTextarea">{t('sections.clipboard.label')}</label> <textarea className="allow-native-input" id="dashboardClipboardTextarea" value={dashboardClipboardContent} onChange={handleClipboardChange} onBlur={handleClipboardBlur} rows="5" placeholder={t('sections.clipboard.placeholder')} /> </div> </div> )}
        </div>
        ...
        <div className="sidebar-section">
             <div className="sidebar-section-header" onClick={() => toggleSection('files')} role="button" aria-expanded={sectionsOpen.files} aria-controls="files-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('files')}>
               <h3>{t('sections.files.title')}</h3> <span className="section-toggle-icon">{sectionsOpen.files ? <CaretUpIcon /> : <CaretDownIcon />}</span>
             </div>
             {sectionsOpen.files && ( <div className="sidebar-section-content" id="files-content"> <button className="resolution-button" onClick={handleUploadClick} style={{ marginTop: '5px', marginBottom: '5px' }} title={t('sections.files.uploadButtonTitle')}> {t('sections.files.uploadButton')} </button> <button className="resolution-button" onClick={toggleFilesModal} style={{ marginTop: '5px', marginBottom: '5px' }} title={t('sections.files.downloadButtonTitle', 'Download Files')}> {t('sections.files.downloadButtonTitle', 'Download Files')} </button> </div> )}
        </div>
        ...
        <div className="sidebar-section">
            <div className="sidebar-section-header" onClick={() => toggleSection('apps')} role="button" aria-expanded={sectionsOpen.apps} aria-controls="apps-content" tabIndex="0" onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('apps')}>
              <h3>{t('sections.apps.title', 'Apps')}</h3> <span className="section-toggle-icon">{sectionsOpen.apps ? <CaretUpIcon /> : <CaretDownIcon />}</span>
            </div>
                        ...
                        {Object.keys(gamepadStates).length > 0 ? (
                            ...
                            <p className="no-gamepads-message">
                                {isMobile ? 
                                    ...
                                    : 
                                    t('sections.gamepads.noActivity', 'No physical gamepad activity detected.')
                                }
                            </p>
                        )}
                    </>
      ...
      <div className={`notification-container theme-${theme}`}>
        {notifications.map(n => (
          <div key={n.id} className={`notification-item ${n.status} ${n.fadingOut ? 'fade-out' : ''}`} role="alert" aria-live="polite">
            <div className="notification-header"> <span className="notification-filename" title={n.fileName}>{n.fileName}</span> <button className="notification-close-button" onClick={() => removeNotification(n.id)} aria-label={t('notifications.closeButtonAlt', { fileName: n.fileName })}>&times;</button> </div>
            <div className="notification-body">
                {n.status === 'progress' && (<> <span className="notification-status-text">{t('notifications.uploading', { progress: n.progress })}</span> <div className="notification-progress-bar-outer"><div className="notification-progress-bar-inner" style={{ width: `${n.progress}%` }} /></div> </>)}
                {n.status === 'end' && (<> <span className="notification-status-text">{t('notifications.uploadComplete')}</span> <div className="notification-progress-bar-outer"><div className="notification-progress-bar-inner" style={{ width: `100%` }} /></div> </>)}
                {n.status === 'error' && (<> <span className="notification-status-text error-text">{t('notifications.uploadFailed')}</span> <div className="notification-progress-bar-outer"><div className="notification-progress-bar-inner" style={{ width: `100%` }} /></div> {n.message && <p className="notification-error-message">{n.message}</p>} </>)}
            </div>
          </div>
        ))}
      </div>
      ...
      {isMobile && (
        <button className={`virtual-keyboard-button theme-${theme}`} onClick={handleShowVirtualKeyboard} title={t('buttons.virtualKeyboardButtonTitle', 'Pop Keyboard')} aria-label={t('buttons.virtualKeyboardButtonTitle', 'Pop Keyboard')}>
          <KeyboardIcon />
        </button>
      )}
    </>
